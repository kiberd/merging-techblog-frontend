...
const Header = () => {
	...
	useEffect(() => {
		...
		if (session && !user.isLogin) setDbUser();
						...
						<div class="flex">
							...
							{session ? (
								<div class="ml-4 flex items-center md:ml-2">
									<button
										type="button"
										className="inline-flex items-center px-3 py-1 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-100 dark:bg-transparent dark:text-white"
									>
					...
					<nav class="flex flex-col sm:flex-row sm:items-center justify-between">
						...
						<div class="py-3 px-6 sm:px-12 lg:px-0">
							<div class="max-w-7xl mx-auto flex items-baseline space-x-4">
								<a
									href="/"
									class={`text-gray-500 dark:text-gray-300 px-3 py-2 text-sm font-medium  ${router.asPath === "/" ? "border-b-2 border-gray-400" : null}`}
									aria-current="page"
								>
									Tranding
								</a>
															...
															(company, index) => (
																<div key={index}>
																	<input
																		...
																		className="float-left w-4 h-4 mt-1 mr-2 align-top transition duration-200 bg-white bg-center bg-no-repeat bg-contain border border-gray-300 rounded-sm appearance-none cursor-pointer form-check-input dark:bg-black dark:checked:bg-gray-600 checked:bg-gray-600 checked:border-gray-600"
																		type="checkbox"
																		value={
																			company.value
																		}
